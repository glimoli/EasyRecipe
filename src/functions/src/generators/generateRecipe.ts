...
export const generateRecipe = onRequest(
  { cors: true },
  async (req, res) => {
    if (req.method !== "POST") {
    ...
    if (maxMinutes) parts.push(`Total time: under ${maxMinutes} minutes`);
    if (dietary) parts.push(`Dietary requirements: ${dietary}`);
    if (ingredients) parts.push(`Must include these ingredients: ${ingredients}`);
    if (avoid) parts.push(`MUST NOT contain any of these ingredients (strictly avoid): ${avoid}`);
    if (freeText) parts.push(`Additional notes: ${freeText}`);
    ...
    try {
      const text = await generateWithGemini(GENERATE_SYSTEM_PROMPT, userPrompt);
      const result = JSON.parse(text);

      if (!result.title) result.title = "AI Recipe";
